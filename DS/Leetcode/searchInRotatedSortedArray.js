function search(nums, target) {
  ...
  while (start <= end) {
    ...
    if (nums[mid] === target) {
      return mid;
    } else if (nums[start] <= nums[mid]) { // left array sorted
      if (target >= nums[start] && target < nums[mid]) { // search target in the left array range
        end = mid - 1;
      } else {
        start = mid + 1;
      }
    } else { // right array sorted
      if (target > nums[mid] && target <= nums[end]) { // search target in the right array range
        start = mid + 1;
      } else {
        end = mid - 1;
      }
